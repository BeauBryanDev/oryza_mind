  ...
  return (
    ...
    <section aria-label="Chat with OryzaMind" className="hud-panel hud-corners p-5 flex flex-col h-full min-h-0 overflow-hidden">
      ...
      <header className="flex items-center gap-2 mb-3">
        ...
        <h2 className="font-display neon-text-green tracking-widest text-lg">2. CHAT WITH ORYZA-MIND</h2>
      </header>
      ...
      <div className="mt-3">
        <PromptBox />
        <p className="text-center text-sm font-hud tracking-[0.15em] text-rg-muted mt-2">
...
function MessageBubble({
  ...
  return (
    ...
    <div className={`flex gap-3 fade-in ${isUser ? 'flex-row-reverse' : ''}`}>
      <div className={`shrink-0 w-12 h-12 rounded-full border flex items-center justify-center
        ${isUser ? 'border-rg-accent/60' : 'border-rg-neon/60 bg-rg-panel2'}`}>
        {isUser ? (
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C7F000" strokeWidth="2"><circle cx="12" cy="8" r="4"/><path d="M4 21a8 8 0 0 1 16 0"/></svg>
        ) : (
          <img src={oryzaChatIcon} alt="AI" width={34} height={34} />
        )}
      </div>
      <div className={`max-w-[85%] min-w-0 rounded-xl px-4 py-2.5 border
        ${isUser
        ...
        <div className="flex items-center gap-3 mb-1">
          ...
          <span className="font-hud text-sm text-rg-muted tracking-[0.2em] tabular-nums">
        ...
        {attachments && attachments.length > 0 && (
          <div className="flex gap-2 mb-2">
            {attachments.map((a, i) => (
              <img key={i} src={a} alt={`attachment ${i + 1}`} className="w-12 h-12 rounded object-cover border border-rg-neon/40" />
            ))}
          </div>
        )}
        ...
        {isUser ? (
          ...
          <div className="text-base leading-[1.55] whitespace-pre-wrap font-body">{content}</div>
        ) : (
